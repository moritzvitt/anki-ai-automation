...
- A cleaner HyperTTS-inspired `Transform with AI` Browser dialog layout with separate workflow pages for source fields, target fields, model settings, prompts, and system prompts, plus clearer step descriptions and more readable helper text.
...
- The user MLR prompt library was regrouped into clearer `mlr default` and `mlr_` folders so the saved prompt browser better matches the way those prompt sets are actually maintained.
