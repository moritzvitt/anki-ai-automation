...
3. When the user right-clicks selected Browser rows, the add-on adds `Transform with AI`.
4. Triggering the action loads config from [`config.py`](../../config.py), resolves selected cards to note IDs, and opens the saved-prompt transform dialog in [`automation_ui.py`](../../automation_ui.py).
5. [`processing.py`](../../processing.py) builds note snapshots, validates prompt placeholders and target fields, and confirms overwrites or appends.
